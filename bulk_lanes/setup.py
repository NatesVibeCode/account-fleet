...
def bundled_skill_path() -> Path:
    ...
    if not (path / "SKILL.md").is_file():
        raise RuntimeError("installed package is missing the bundled bulk-lanes skill")
    return path
...
def installed_cli_path() -> str:
    sibling = Path(sys.executable).absolute().parent / "bulk-lanes"
    discovered = shutil.which("bulk-lanes")
    if sibling.is_file():
        return str(sibling)
    return str(Path(discovered).resolve()) if discovered else "bulk-lanes"
...
def setup_workspace(
    *,
    ...
    database = Path(db_path).expanduser() if db_path is not None else Path("bulk-lanes.db")
    ...
    if not refresh_routes or observed_route_count == 0:
        next_commands.append(["bulk-lanes", "routes", "--db", str(database), "--refresh", "--json"])
    next_commands.extend([
        ["bulk-lanes", "doctor", "--db", str(database), "--workspace-root", str(workspace), "--json"],
        ["bulk-lanes", "init", "my-task", "--db", str(database), "--preset", "classify"],
    ])
    return SetupReport(
        ready=ready,
        scope=scope,
