...
account-fleet runs entirely on models that cost **$0**. You do not need a paid account, and you can
...
OpenRouter is a gateway that serves free models from many providers behind one key. account-fleet already
...
3. Give the key to account-fleet:

   ```bash
   export OPENROUTER_API_KEY="sk-or-paste-yours-here"
   account-fleet doctor
   ```
...
work on hundreds of records, not for millions. Run `account-fleet routes` to see exactly what is available
to you right now.
...
OpenCode ships free hosted models of its own; they appear in `account-fleet routes` as `opencode/...`.
...
account-fleet routes add ollama/llama3.2 --provider ollama --free
```
...
```bash
account-fleet doctor     # is each AI tool installed? is your OpenRouter key set?
account-fleet routes     # the $0 routes available right now (add --all to see everything)
account-fleet studio     # click-to-choose page for picking which tools and models runs may use
```
