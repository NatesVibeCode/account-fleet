...
from free_fleet.cli import build_parser, cmd_mcp_install
...
def test_repeated_rate_limits_do_not_exhaust_batch_budget(tmp_path):
    store = FreeFleetStore(tmp_path / "fleet.db")
...
def test_mcp_install_preserves_invalid_existing_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("free_fleet.cli._existing_mcp_path_for_client", lambda *a: config)
...
def test_client_config_default_matches_operating_system(tmp_path, monkeypatch, platform, subpath):
    from free_fleet.cli import _claude_config_candidates
    monkeypatch.setattr("free_fleet.cli.sys.platform", platform)
...
def test_different_worker_timeouts_do_not_close_an_active_http_client(monkeypatch, name):
    import importlib
    module = importlib.import_module("free_fleet.providers." + name)
