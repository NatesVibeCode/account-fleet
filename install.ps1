<#
.SYNOPSIS
  One-click installer for account-fleet (Windows).

.DESCRIPTION
  Creates a private Python environment in this repo's `.venv`, installs the
  account-fleet CLI into it, sets up a workspace, runs the offline demo, and
  registers the account-fleet MCP tools with Claude Desktop (or Cursor).

  Usage:
    .\install.ps1 [workspace-directory]

  The workspace defaults to ~\account-fleet-workspace when no argument is given.
...
$cli     = Join-Path $venvDir "Scripts\account-fleet.exe"
...
Say "Installing account-fleet (this can take a minute the first time)..."
...
if ($pipCode -ne 0) {
  Fail "Installing account-fleet failed. See $log for details. (Usually a network problem reaching PyPI.)"
}

& $venvPy -c "import harness_fleet" *>> $log
if ($LASTEXITCODE -ne 0) {
  Fail "account-fleet installed but could not be imported. See $log for details."
}
Ok "account-fleet installed at $cli"

# --- 3. choose and create the workspace -------------------------------------

Say ""
Say "Step 3/8 - Creating the workspace"
$ws = if ($args.Count -ge 1) { $args[0] } else { Join-Path $HOME "account-fleet-workspace" }
...
# The shared engine's default database filename is harness-fleet.db, so keep it:
# a later bare `account-fleet status` (no --db) resolves to the same file.
...
Say ""
Say "Step 7/8 - Registering account-fleet with Claude Desktop / Cursor"
...
Say ""
Say "--------------------------------------------------------------"
Say "  account-fleet is ready!"
Say "--------------------------------------------------------------"
Say ""
Say "  Installed:"
Say "    Python environment : $venvDir"
Say "    account-fleet CLI  : $cli"
Say "    Workspace          : $ws"
Say "    Demo CSV           : $demoCsv"
Say ""
Say "  Next steps:"
Say "    1. Restart Claude Desktop (or Cursor) now to load the account-fleet tools."
