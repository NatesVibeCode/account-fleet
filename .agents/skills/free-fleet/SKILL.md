...
4. Create or select a task (`--preset score|filter|account-research|classify|extract|triage|summarize` or `--from-example labels.csv`), then run `free-fleet validate TASK --input FILE [--only-ids FILE] --json` before inference. Long documents are tri-window sliced (head/mid/tail) with `partial:true` and exact offsets; `validate` warns when truncation occurs.
...
10. Export and validate the packet with `free-fleet export RUN_ID [--format json|csv|jsonl] [--sort-by CLAIM] [--desc] [--top N] [--rank] [--filter EXPR]`. Use `free-fleet db backup <path>` for safe SQLite copies. Report database path, run ID, packet path, verified/failed counts, attempts, and route/cost evidence.
